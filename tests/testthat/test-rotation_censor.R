# rotation_censor, including the center_offset enhancement: it must accept an
...
test_that("rotation_censor returns a fixed-width window", {
  ...
  out <- rotation_censor(img, center_offset = 20, fixed_width = 10,
                         fixed_rotation = TRUE)
...
test_that("fraction and keyword agree with the equivalent absolute row", {
  ...
  by_row     <- rotation_censor(img, center_offset = 20,       fixed_width = 10)
  by_frac    <- rotation_censor(img, center_offset = 0.5,      fixed_width = 10)
  by_keyword <- rotation_censor(img, center_offset = "middle", fixed_width = 10)
...
test_that("rotation_censor rejects unknown keywords", {
  ...
  expect_error(rotation_censor(img, center_offset = "sideways"))
})
