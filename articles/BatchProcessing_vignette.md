...
tray says nothing about where it grew, so the whole scan is summarised
as one row.
...
| **Set for raw scans** | `binarize_threshold`, `dark_roots` | Only if your files are greyscale or RGB rather than a binary mask. Both accept one value per image, for sessions where exposure drifted. |
...
The default, `"rounding"`, labels a bin by its centre: with
...
convention: 0–5, 5–10, labelled by the shallower edge. Prefer it for new
