...
Minirhizotron cameras often capture a tube (or a long
...
`?stitch_image_sequence` for continue ; `?list_tubes` and `?list_scan_files`
preview a folder.)
...
`overlap = edge_width - dx`, set it to roughly **2x your true overlap** (the
...
`vertical_offset` to skip a header, label, or tape strips, or other artefacts at the edges (some scanners produce white edge lines).
...
message rather than guessing. If you insist on feature matching, see the GitHub repository: [ImageStitching](https://github.com/jcunow/ImageStitching)
...
Frames are stitched in **sorted filename order**, so name them accordingly.
`stitch_root_scans()` also accepts a plain character vector of paths.
...
`vertical_offset` past a header/tape strip, try different preprocessing options e.g., `preprocess = "grad"` for uneven
lighting, or confirm the frames are in the intended (filename) order. Check if the order 
