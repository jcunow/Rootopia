...
# RootScanR + terra are hard dependencies, so the workflow below can run at
...
> * **One slice at a time** with `zoning()` — for traits whose function needs a
>   whole image (root length, landscape metrics, colour). You compute the trait
...
whole raster rather than a per-pixel reduction. `zoning()` masks the skeleton
...
len_10cm <- root_length(skl_10cm, unit = "cm", dpi = dpi, show_messages = FALSE)
len_10cm
```
...
Both of these are computed **per depth slice** with the same `zoning()` loop
introduced in 5c.
...
`tube_coloration()` is the colour extractor here — exactly as in the
...
t1 <-  load_flexible_image(seg_Oulanka2023_Session01_T067, "spatrast")
t2 <-  load_flexible_image(seg_Oulanka2023_Session03_T067, "spatrast")
