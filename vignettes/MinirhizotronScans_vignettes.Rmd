...
> **A note on figures.** Minirhizotron scans are several thousand pixels wide.
> Shrinking one to fit an html_vignette figure forces the graphics device to
> drop one-pixel-wide roots and skeletons, so they appear broken or vanish.
> The figures below therefore use `zoom_plot()` (a Rootopia function): it draws
> a full-image **overview** with the magnified region outlined, plus a
> **native-resolution inset** where thin features survive at ~1:1. `frac` sets
> the fraction of each axis kept in the inset (magnification `1/frac`); `center`
> controls which part is magnified (`"center"`, `"densest"`, or relative
> `c(fx, fy)`).
...
> workflow is the recommended starting point. Once you are comfortable with
> the individual functions and want to process a whole folder of images at
> once, the [Batch Processing](BatchProcessing_vignette.html) vignette wraps
> this entire workflow into a single `root_depth_metrics()` call.
...
`root_diameter()` estimates a diameter at every skeleton pixel (in the `unit`
you specify) and returns it as a raster. Because it is a per-pixel value, the
profile reduces with `zonal()` just like the pixel counts in 5a.
...
> Looking for **root branching order** (segments grouped by branching order)?
> That analysis has no depth dimension, so it lives in the
