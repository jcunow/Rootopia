...
> If a tube consists of multiple overlapping images, consider stitching [Image Stitching](Stitching_vignette.html)
...
```{r fig.alt=c("Binary root layer of a minirhizotron scan, roots in white, with the magnified region outlined.", "The same root layer magnified to native resolution.")}
...
```{r fig.alt="Depth map in centimetres, shading from shallow at one end to deep at the other."}
...
width (`nn`) should match your analysis scale — 5 cm is a common choice. You may want to adjust this depending on your root prevalence.  

```{r fig.alt="The depth map reduced to discrete depth bins, drawn as bands of constant colour."}
...
```{r fig.alt="Root trait plotted for each depth bin down the profile."}
...
```{r fig.alt="Cumulative root accumulation against depth."}
