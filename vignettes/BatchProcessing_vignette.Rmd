...
`mrd`, `rpi`, `total.length.density`, `rootlength.fraction`,
...
each image (via `branch_order_map()`) and classifies every root segment by
its **branch order** — the thickest, most central root in each connected
component is order 1 (the "main" axis), its laterals are order 2, their
laterals order 3, and so on. See the
[Minirhizotron Scans](MinirhizotronScans_vignettes.html#6-root-branching-order)
...
**Per tube** (repeated on every row of that tube) — main root vs. lateral
roots, split by `order_metrics(..., focal = "thickest")`:
