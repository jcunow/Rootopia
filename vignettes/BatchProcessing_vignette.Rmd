...
| Skeletonised images (one-pixel centrelines) — *optional* | RootDetector (channel 2), or omit `path.skl` and `root_depth_metrics()` will compute skeletons internally via `skeletonize_image()` — needed for length, diameter, angle, and branching-order metrics |
...
This is the fastest way to get going. Only `path.seg` is required; everything
else has sensible defaults. `path.skl` is optional — if omitted, skeletons
